...
try {
  ...
  if (is_ajax()) json_out(true, 'Data kelas berhasil disimpan.', 'success');
  header('Location: data_kelas.php?msg=' . urlencode('Data kelas berhasil disimpan.'));
  exit;
} catch (Throwable $e) {
  if (is_ajax()) json_out(false, 'Terjadi kesalahan saat menyimpan data.', 'danger', 500);
  header('Location: data_kelas.php?err=' . urlencode('Terjadi kesalahan saat menyimpan data.'));
  exit;
}
