...
// id_guru dari session (boleh null kalau bukan guru)
$id_guru = isset($_SESSION['id_guru']) ? (int)$_SESSION['id_guru'] : null;
...
// Ambil id_sekolah aktif (kalau ada)
$id_sekolah = isset($sekolah['id_sekolah']) ? (int)$sekolah['id_sekolah'] : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $catatan_baru = isset($_POST['catatan'])
    ? trim($_POST['catatan'])
    : '';

  $sqlCR = "
    INSERT INTO cetak_rapor
      (id_guru, id_siswa, id_pengaturan_cetak_rapor, id_sekolah, catatan_wali_kelas)
    VALUES (?, ?, ?, ?, ?)
    ON DUPLICATE KEY UPDATE
      id_guru = VALUES(id_guru),
      id_pengaturan_cetak_rapor = VALUES(id_pengaturan_cetak_rapor),
      id_sekolah = VALUES(id_sekolah),
      catatan_wali_kelas = VALUES(catatan_wali_kelas)
  ";

  $stmtSave = $koneksi->prepare($sqlCR);
  if (!$stmtSave) {
    die('Prepare gagal: ' . $koneksi->error);
  }

  // pastikan NULL benar-benar NULL
  $id_guru_param     = $_SESSION['id_guru'] ?? null;
  $id_peng_param     = $id_pengaturan_cetak_rapor ?: null;
  $id_sekolah_param  = null;
  ...
  if ($stmtSave->execute()) {
    $catatan_walas = $catatan_baru;
    echo "<script>alert('Catatan berhasil disimpan'); window.location.href = 'data_siswa.php';</script>";
  } else {
  ...
  <div class="page">
        ...
        <tbody>
          ...
          $renderKelompok = function ($label, $rows) use (&$no) {
            ...
            $prioritas = [
              ...
              // nanti bisa tambah rumpun lain di sini jika dibutuhkan
            ];
    ...
    <section class="bottom">
      <div class="left">
        ...
        <form method="post">
          <div class="catatan">
            <textarea name="catatan" id="catatan" class="catatan" style="width: 100%;" autofocus><?= htmlspecialchars($catatan_walas); ?></textarea>
          </div>
